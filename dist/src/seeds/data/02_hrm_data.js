...
exports.hrmDataSeed = {
    ...
    run: async () => {
        ...
        const allUsers = await db_1.db.select().from(schema_1.users).limit(2);
        ...
        const existingHolidays = await db_1.db.select().from(schema_1.holidays);
        if (existingHolidays.length === 0) {
            ...
            await db_1.db.update(schema_1.holidays).set({
                type: "fixed",
                days: ["Friday", "Saturday"]
            }).where((0, drizzle_orm_1.eq)(schema_1.holidays.id, existingHolidays[0].id));
        }
        ...
        await db_1.db.insert(schema_1.attendance).values(attendanceData);
    ...
    rollback: async () => {
        await db_1.db.delete(schema_1.attendance);
        await db_1.db.delete(schema_1.holidayRequests);
        await db_1.db.delete(schema_1.onlineRequests);
        await db_1.db.delete(schema_1.permissions);
        await db_1.db.delete(schema_1.holidays);
    }
};
